...
process_scan (IIOSensorData data, DrvData *or_data)
{
	...
	process_scan_1(data.data + or_data->buffer_data->scan_size*i, or_data->buffer_data, "in_accel_y", &accel_x, &scale, &present_x);
	process_scan_1(data.data + or_data->buffer_data->scan_size*i, or_data->buffer_data, "in_accel_x", &accel_y, &scale, &present_y);
